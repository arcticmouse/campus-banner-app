...
$thisService = 'https://publicaffairs.berkeley.edu/campus-banners' . $_SERVER['PHP_SELF'];
...
if ($_SERVER["REQUEST_METHOD"] && $_GET["ticket"]) {
   if ($response = responseForTicket($_GET["ticket"])) {
      if ($uid = uid($response)) {
         error_log("The user is: $uid");
      }
      else {
         error_log("Could not get UID from response.");
      }
   }
   else {
      error_log("The response was not valid.");
   }
}
else {
...
/*
$casService = 'https://auth-test.berkeley.edu/cas';
//$thisService = 'https://publicaffairs.berkeley.edu' . $_SERVER['PHP_SELF'];
$thisService = 'https://publicaffairs.berkeley.edu/campus-banners/';
error_log('request method is ' . $_SERVER["REQUEST_METHOD"] . ' ticket is ' . $_GET["ticket"]);
header("Location:$casService/login?service=$thisService");
*/
?>
